...
// Bytes type, represents a chunk of byte-sized values. 
struct bytes_t 
{
	...
	// memory. dflt is never used again once the switch occurs.
	uint8_t dflt[DEFAULT_SIZE];

	// Points to either dflt, or a dynamically allocated chunk of bytes.
	uint8_t *data;
	
	// The number of slots in use, initially 0.
	size_t usage;
...
// 
// Post:
//	data - Points to the default buffer, dflt.
...
// Accessor function, gets the raw uint8_t pointer, bytes->data.
//
// Pre:
//	bytes - Points to valid bytes_t variable and is not NULL.
//
...
//
// Post:
//	If index < usage, then the byte at bytes->data[index] has been
//	set to val.
//
// Returns:
